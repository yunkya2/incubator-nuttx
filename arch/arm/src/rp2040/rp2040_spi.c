  ...
  if (nbits != priv->nbits)
    {
      ...
      if (priv->nbits > 8)
        {
          priv->txconfig.dest_width = RP2040_DMAC_WIDTH16;
          priv->txconfig.src_width = RP2040_DMAC_WIDTH16;
          priv->rxconfig.dest_width = RP2040_DMAC_WIDTH16;
          priv->rxconfig.src_width = RP2040_DMAC_WIDTH16;
        }
      else
        {
          priv->txconfig.dest_width = RP2040_DMAC_WIDTH8;
          priv->txconfig.src_width = RP2040_DMAC_WIDTH8;
          priv->rxconfig.dest_width = RP2040_DMAC_WIDTH8;
          priv->rxconfig.src_width = RP2040_DMAC_WIDTH8;
        }
#endif
    }
}
  ...
  while (nwords || rxpending)
    {
      ...
      while ((spi_getreg(priv, RP2040_SPI_SSPSR_OFFSET) & RP2040_SPI_SSPSR_TNF) &&
             (rxpending < RP2040_SPI_FIFOSZ) && nwords)
        {
          ...
          spi_putreg(priv, RP2040_SPI_SSPDR_OFFSET, txbuffer ? data : datadummy);
          nwords--;
          rxpending++;
        }
...
{
  FAR struct rp2040_spidev_s *priv = NULL;

  switch (port)
    {
#if defined(CONFIG_RP2040_SPI4) && defined(CONFIG_RP2040_DMAC_SPI4_TX)
      case 4:
        priv = &g_spi4dev;
        break;
#endif

#if defined(CONFIG_RP2040_SPI5) && defined(CONFIG_RP2040_DMAC_SPI5_TX)
      case 5:
        priv = &g_spi5dev;
        break;
#endif
  ...
  if (priv && priv->initialized)
    {
      if ((chtype == RP2040_SPI_DMAC_CHTYPE_TX) && (!priv->txdmach))
        {
          /* TX DMA setting */
      ...
      else if ((chtype == RP2040_SPI_DMAC_CHTYPE_RX) && (!priv->rxdmach))
        {
          /* RX DMA setting */
...
static void spi_dmatxcallback(DMA_HANDLE handle, uint8_t status, void *data)
{
  ...
  if ((status & RP2040_DMA_INTR_ERR) != 0)
    {
      spierr("dma error\n");
    }
