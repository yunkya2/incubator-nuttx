...
#define RP2040_TRACEINTID_GETIFDEV        9
#define RP2040_TRACEINTID_GETENDPOINT    10
#define RP2040_TRACEINTID_HANDLEZLP      11
#define RP2040_TRACEINTID_USBINTERRUPT   12
#define RP2040_TRACEINTID_INTR_BUSRESET  13
#define RP2040_TRACEINTID_INTR_BUFFSTAT  14
                          ...
                          uint16_t nbytes)
{
  ...
  bytesleft = RP2040_BULKMAXPACKET;
  if (bytesleft > nbytes)
    {
      bytesleft = nbytes;
    }

  usbtrace(TRACE_WRITE(privep->epphy), bytesleft); /* TBD */

  /* Copy the transmit data into DPSRAM */

  memcpy(privep->data_buf, buf, bytesleft);
...
static void rp2040_ep0setup(FAR struct rp2040_usbdev_s *priv)
{
  ...
  FAR struct rp2040_req_s *privreq = rp2040_rqpeek(ep0);
  FAR struct rp2040_ep_s *privep;
  uint16_t index;
  uint16_t value;
  uint16_t len;
 ...
 *
 * Description:
 *   Handle the end of EP data transfer
 *
 ****************************************************************************/
...
static void rp2040_usbintr_epdone1(FAR struct rp2040_usbdev_s *priv,
                                  int epindex)
{
  struct rp2040_req_s *privreq;
  struct rp2040_ep_s *privep;
...
//  *(volatile uint32_t *)0x50100084 = 0x2440;
...
static int rp2040_pullup(FAR struct usbdev_s *dev, bool enable)
{
  ...
  if (enable)
  ...
  else
    {
      ...
      putreg32(0, g_usbdev.eplist[1].buf_ctrl);
    }
